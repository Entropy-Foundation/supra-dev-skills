---
name: supra-ts-sdk-skill
description: |
  Guide for integrating the Supra L1 blockchain TypeScript SDK (supra-ts-sdk) into React, Next.js or Node projects. Covers client setup, account queries, coin/fungible asset transfers, smart contract interaction via ABI proxies, transaction lifecycle (build/simulate/submit/wait), view functions, events, and Move type mapping. Use when the user asks to integrate Supra into a React/Next.js/Node app, query Supra accounts, balances or transactions from code, build/sign/submit Supra transactions, call Move contracts from TypeScript, or asks about supra-ts-sdk usage. This file mirrors the skill bundled inside the supra-ts-sdk npm package (.claude/skills/supra-ts-sdk/SKILL.md); when the SDK releases a new version, re-sync this file from there.
---
...
const txn: TransactionResponse = await supra.coin.transferSupraCoin({
    ...
    amount: 10000,  // in smallest unit (Quants; 1 SUPRA = 100_000_000 Quants)
...
### Signing with StarKey instead of a SupraAccount

In a browser dApp the private key lives in the wallet, so build with the SDK and let StarKey sign and submit:

```typescript
const rawTxn = supra.transaction.build.rawTxnObject({ /* as above; senderAddress = the connected account */ });
...
const txHash = await window.starkey.supra.sendTransaction({
    ...
    to: "",
    value: "",
    chainId: "6",           // "6" testnet, "8" mainnet - strings
});
```

StarKey's `createRawTransactionData()` is deprecated; this is the replacement. The full hook, connect UI and sign-in flow live in the `supra-wallet-connect-skill`.
