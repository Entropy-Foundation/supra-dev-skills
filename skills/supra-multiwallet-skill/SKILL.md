...
- Optionally: a full **sign-in-with-wallet â JWT â httpOnly cookie** auth flow with nonce/signature verification and edge-runtime API routes
...
3. **Do they already have a wallet integration?** If they have Starkey-only code, this is a migration â preserve their existing wallet state consumers and layer Ribbit support on top. Read `references/migrating-from-single-wallet.md`.
...
All files referenced below live in this skill's `assets/` directory. Read them with the `view` tool and write them into the target project with `create_file`. Do not try to reconstruct them from memory â they contain carefully-handled edge cases (wallet event listeners, localStorage fallbacks, Ribbit SDK initialization timing) that are easy to break.
...
- `STORAGE_KEY = 'multiwallet.selectedWallet'` â optional, but namespacing it to the project (e.g. `'myapp.selectedWallet'`) avoids collisions if the user visits multiple Supra dApps.
...
- Branding strings like `'MyDApp'`, `'Welcome back to MyDApp'`, `'Â© Powered by Crystara'` should be updated.
...
If auth is not being used, `JWT_SECRET` can be skipped â but `lib/auth.ts` throws on import if it's missing, so either set a placeholder or don't import the file.
...
export function MyComponent() {
  const {
    accounts,          // string[] â connected address(es); accounts[0] is the active one
  ...
  return <p>Connected: {accounts[0]} â {balance}</p>;
}
```

Or, for the full drop-in modal with a "Connect Wallet" button that shows both wallets side-by-side, use `ConnectWalletHandler` as a render-prop wrapper â see `references/using-connect-wallet-handler.md`.
...
2. Click connect â select a wallet â approve in the wallet â sign the message
3. Check DevTools â Application â Cookies for `authToken` being set as httpOnly
4. Check the network tab: `/api/auth/nonce` â `/api/auth/create-jwt` â `/api/auth/wallet-login` should all return 200
...
- **`references/hook-api.md`** â full signature and behavior of every method returned by `useSupraMultiWallet`. Read when the user asks about a specific method or wants to build custom UI around the hook.
- **`references/sending-transactions.md`** â how `sendRawTransaction` works across both wallets, BCS argument serialization, type args, chain selection. Read when implementing token transfers or Move function calls.
- **`references/auth-architecture.md`** â the nonce/JWT/signature flow in detail, why each piece exists, and how to customize expiration windows, the auth message, or the revalidation cadence. Read when modifying auth behavior.
- **`references/no-auth-mode.md`** â how to strip the JWT auth out of the hook if the project only needs wallet connection (no sign-in). Read when the user explicitly says they don't want sign-in or when integrating into a read-only dApp.
- **`references/using-connect-wallet-handler.md`** â how to use `ConnectWalletHandler` as a render-prop wrapper, customize the modal, and handle `onConnect`/`onDisconnect` callbacks.
- **`references/migrating-from-single-wallet.md`** â how to migrate an existing Starkey-only (or Ribbit-only) project to support both. Read when the project already has wallet code.
- **`references/troubleshooting.md`** â common failures (signature verification failing, Ribbit not initializing, infinite "connecting" state, CORS on edge routes) and their fixes.

---

## Key things to remember

- **Starkey and Ribbit behave differently.** Starkey is a browser extension (synchronous detection via `window.starkey?.supra`), supports account switching events, network switching, and emits `starkey-*` window messages. Ribbit is an SDK that's initialized via `initSdk()` and uses its own internal state â it does *not* support network switching (the user must switch in-app) or account-switch events. The hook abstracts this via the `WalletCapabilities` object and capability guards â respect those guards if extending the hook.
...
- **The hook is long (~1,200 lines) for good reasons.** It handles wallet-install polling, storage fallbacks (localStorage â sessionStorage â cookie), starkey-wallet-updated events that require re-authentication, and graceful degradation when a wallet lacks a capability. Resist the urge to "clean it up" without understanding what each section does.
