...
- The sign-in message string `'Sign message to login to multiwallet. By signing this message, you agree to the Terms of Service and Privacy Policy of multiwallet at https://multiwallet.trade/tos'` appears **three times** (initial connect for Starkey, initial connect for Ribbit, and in the `signIn()`/revalidation flows). Replace with the target project's name and TOS URL. **The exact same string must also be set as `AUTH_MESSAGE` in `app/api/auth/create-jwt/route.ts`** — if these drift apart, signature verification fails and no one can log in.
