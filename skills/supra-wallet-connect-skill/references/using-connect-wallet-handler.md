...
`onConnect(address)` fires from the hook's `wallet-connected` event, so it receives the account that was actually authenticated. It also fires after a successful re-authentication and after an account switch in StarKey, with the new address. Earlier versions read `accounts[0]` right after `await connectWallet()`, which is the pre-connect value and empty on a first connect, so the callback never ran.
