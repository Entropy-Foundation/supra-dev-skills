...
`WalletProvider` calls `useSupraWallet()` once and exposes the result through
context. Inside the provider, read it with `useSupraWalletContext()` from
`components/WalletProvider.tsx` rather than calling `useSupraWallet()` again -
every extra call is another provider subscription, another detection poll and
another retried account read. `useSupraWalletWithRefresh()` is a deprecated
...
7. Call `signMessage(authMessageFor(nonce), nonce, address)` — wallet shows the sign prompt. The nonce is inside the signed text; see `auth-architecture.md`
...
### `signMessage(message: string, nonce?: string, account?: string, forceSign?: boolean) => Promise<{ signature, publicKey, address?, verified } | undefined>`

Signs an arbitrary UTF-8 message. Returns StarKey's `{ address, publicKey, signature }` plus `verified`, the local Ed25519 verification result (a boolean; should always be `true` if the wallet signed correctly).

`nonce` is **not** sent to the wallet. StarKey's `signMessage` takes only `{ message }` and signs exactly those bytes, so anything that must be bound to the signature has to be inside `message` - which is what `authMessageFor(nonce)` does for the login flows. The parameter remains for source compatibility with earlier copies of the hook.

`account` is the address to treat as the signer when `accounts` has not committed yet (a just-connected wallet). It is never written back into state.

Uses a `localStorage.isSigningWallet = 'true'` reentrancy guard to prevent double-prompts; `forceSign=true` bypasses this. The guard is reset on success and on a thrown error (a declined prompt), so one rejection does not lock out later attempts.

The message is hex-encoded (`0x` + hex) before being passed to the wallet — this is what Starkey expects.

### `sendRawTransaction(moduleAddress, moduleName, functionName, params, runTimeParams, txExpiryTime?) => Promise<string | null | undefined>`
...
session did not authenticate, verifies the chain via `ensureChain`, then builds
the raw transaction with `supra-ts-sdk` (`supra.transaction.build.rawTxnObject`,
sequence number from `supra.account.getAccountInfo`) and hands the hex-encoded
bytes to `provider.sendTransaction({ data, from, to: '', value: '', chainId })`.
StarKey's `createRawTransactionData()` is deprecated in its docs and is not used.
`runTimeParams` may be type-tag strings or `TxnBuilderTypes.TypeTag` objects.
Resolves to `null` if the user declines in the wallet.
...
Explicit re-sign flow — useful after a manual token invalidation (e.g. server-side session revocation) to regenerate a JWT without fully disconnecting the wallet. Signs `authMessageFor(nonce)`, posts to `/api/auth/create-jwt`, then to `/api/auth/wallet-login` so the server sets the httpOnly cookie (the same path `connectWallet` uses). Returns `true` when a new session was minted.
