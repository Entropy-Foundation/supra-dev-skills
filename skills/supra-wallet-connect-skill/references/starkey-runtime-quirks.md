...
Sources: [StarKey — Supra provider API reference](https://docs.starkey.app/supra/api-reference),
[StarKey — Events](https://docs.starkey.app/supra/events),
[StarKey — Signing messages](https://docs.starkey.app/supra/signing-messages),
[Supra — Create a dApp with StarKey](https://docs.supra.com/network/move/getting-started/your-first-dapp-with-starkey).
The older `docs.starkey.app/getting-started/*` pages no longer exist.
...
The first `accounts()` after a page load routinely resolves `[]` on a wallet that
is connected and unlocked. (`accounts()` is the current method; `account()` is
deprecated, and the hook falls back to it only on builds that lack `accounts()`.)
...
## 7. Two provider methods are deprecated

`account()` and `createRawTransactionData()` are marked deprecated in the API
reference. Use `accounts()`, and build transactions with `supra-ts-sdk`
(`supra.transaction.build.rawTxnObject(...).toBytes()` as hex in
`sendTransaction.data`). Both still work on current builds, which is why code
that uses them looks fine right up until it does not.

---

## Signing, while you are here

Three things about the auth flow that bite in the same debugging sessions:

- **The wallet signs only the bytes you hand it.** `signMessage({ message })`
  mixes in no nonce and no prefix, so a login nonce protects nothing unless it is
  inside the message text. Sign `authMessageFor(nonce)` from
  `lib/auth-constants.ts`, and verify the same composition on the server.
- **The server verifies exactly one composition.** Any "revalidate" or
  "token expiry" wording signed on the client can never verify. Import
  `authMessageFor` at every call site, client and server, and sign that one
