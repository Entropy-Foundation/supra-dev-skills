...
so the first `accounts()` after a page load routinely resolves `[]` on a wallet
...
Import `authMessageFor` from `lib/auth-constants.ts` at every call site — the
connect path, `signIn()`, `checkAndRevalidateToken()`, the account-switch
handler, and the `create-jwt` route — and sign that one composition everywhere.
Grep for any remaining literal auth message in the project; there should be none.
...
## A captured signature keeps working as a login

**Security.** If the client signs the bare `AUTH_MESSAGE` and only sends the
nonce alongside it, the nonce is decorative: StarKey signs exactly the bytes it
is given, so nothing ties the signature to the nonce the server validates. One
captured `{ signature, publicKey }` then mints a JWT for that address with any
fresh nonce, forever.

Both sides must use `authMessageFor(nonce)`:

```ts
// client
await signMessage(authMessageFor(nonce), nonce, address, true);
// server (create-jwt)
await verifyWalletSignature(authMessageFor(nonce), signature, address);
```

If you ported an older copy, grep for `signMessage(AUTH_MESSAGE` and
`verifyWalletSignature(AUTH_MESSAGE` - both should return nothing.

## "Invalid signature. Signature verification failed." (401 from `/api/auth/create-jwt`)

**By far the most common issue.** The text signed on the client doesn't exactly match what the server verifies. Even a single trailing space, a `\n`, or a different TOS URL will cause `nacl.sign.detached.verify` to return `false`.

Checklist:
- Every client sign-in path signs `authMessageFor(nonce)` and `create-jwt` verifies `authMessageFor(nonce)` with the **same** nonce the client sent. A bare `AUTH_MESSAGE` on either side is a mismatch.
- `APP_NAME` and `TOS_URL` were edited only in `lib/auth-constants.ts`; no other file declares the string or the nonce format.
- Both the client bundle and the server were rebuilt after the edit (Next.js snapshots env and constants at build time).
...
- If the re-auth is succeeding but the UI isn't refreshing, check that the component reads wallet state from `useSupraWalletContext()` (or its own `useSupraWallet()`) and that `WalletProvider` wraps it. Server Components additionally need `WalletSessionSync` (see above)
