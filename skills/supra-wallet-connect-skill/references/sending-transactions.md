...
`sendRawTransaction` is the API for Move entry function calls. It builds the raw transaction with `supra-ts-sdk` and hands the serialized bytes to StarKey's `sendTransaction`, which signs and submits. StarKey's own `createRawTransactionData()` is deprecated in its docs and is not used.
...
SUPRA has 8 decimals. Other coins may not: read `decimals` from `supra.coin.getCoinInfo({ coinType })` before scaling a non-SUPRA amount.
...
const supra = new SupraClient({ network: Network.TESTNET });            // or MAINNET
...
const rawTxn = supra.transaction.build.rawTxnObject({
  senderAddress: sender,                                               // accounts[0]
  senderSequenceNumber: sequence_number,                               // bigint
  function: `${moduleAddress}::${moduleName}::${functionName}`,
  functionTypeArgs: runTimeParams.map((t) => new TypeTagParser(t).parseTypeTag()),
  functionArgs: params,                                                // Uint8Array[]
  optionalTransactionPayloadArgs: { txExpiryTime: BigInt(txExpiryTime) },
});

const txHash = await provider.sendTransaction({
  data: Buffer.from(rawTxn.toBytes()).toString('hex'),                 // serialized raw txn, hex
  from: sender,
  to: '',                                                              // unused for Move calls
  value: '',
  chainId: TARGET_CHAIN_ID,
});
```

The sequence number comes from the SDK, so the payload the wallet signs is exactly the payload the chain executes. `sendTransaction` resolves to the hash, or `null` if the user declined.
