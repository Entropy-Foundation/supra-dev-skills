 ...
 *   const text = authMessageFor(nonce);   // what the wallet signs and the server verifies
 *
...
/**
 * The exact text the wallet signs for one login attempt: AUTH_MESSAGE with the
 * server-issued nonce appended.
 *
 * WHY THE NONCE IS INSIDE THE MESSAGE:
 * StarKey's signMessage signs only the bytes it is given - it does not mix in
 * a nonce or any prefix. If the client signed the bare AUTH_MESSAGE, one
 * captured { signature, publicKey } pair would be a permanent bearer
 * credential: anyone holding it could fetch a fresh nonce and mint a JWT for
 * that address forever, because the nonce the server validates would have no
 * connection to what was signed. Putting the nonce in the signed text ties the
 * signature to that one nonce, and therefore to its 5-minute validity window.
 *
 * Both sides MUST use this function: every client sign-in path and the
 * create-jwt route. Never reconstruct the format by hand.
 */
export function authMessageFor(nonce: string): string {
    return `${AUTH_MESSAGE}\n\nNonce: ${nonce}`;
}
