...
/**
 * SDK client for the target chain. Used only to build raw transaction bytes
 * (sequence number lookup + BCS payload); signing and submission stay in the
 * wallet. Built lazily and once - one client per page, not per call.
 */
let sdkClient: SupraClient | null = null;
const getSupraClient = (): SupraClient => {
  if (!sdkClient) {
    sdkClient = new SupraClient({
      network: TARGET_CHAIN_ID === '8' ? Network.MAINNET : Network.TESTNET,
    });
  }
  return sdkClient;
};
 ...
 * answer, so the first `accounts()` after a page load routinely resolves empty
...
const readStarkeyAccount = async (
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    // accounts() is the documented method; account() is deprecated but older
    // builds only ship that one, so fall back to it.
    const read: Promise<string[]> =
      typeof provider.accounts === 'function'
        ? provider.accounts()
        : provider.account();
    const accounts: string[] = await Promise.resolve(read).catch(
      () => [] as string[]
    );
...
const useSupraWallet = (options: UseSupraWalletOptions = {}) => {
  ...
  const connectWallet = async () => {
    ...
    try {
      ...
      // network. StarKey documents connect({ chainId?, multiple? }); an older
      // build that ignores the argument simply drops it, which is why
      // ensureChain still runs below either way.
      ...
      // Authentication flow. authMessageFor(nonce) is the single source of
      // truth shared with app/api/auth/create-jwt - the server verifies it
      // byte-for-byte, so a local copy of the string here is a silent 401
      // waiting to happen. The nonce is INSIDE the signed text on purpose:
      // StarKey signs only the message it is given, so a signature over a
      // constant string would be a permanent, replayable login credential.
  ...
  const sendRawTransaction = async (
    ...
    try {
      ...
      // Build the raw transaction with the SDK, not with StarKey's
      // createRawTransactionData() - that provider method is deprecated in the
      // StarKey docs, which point at the SDK builder instead. The wallet still
      // signs and submits; the SDK only produces the BCS bytes.
      const supra = getSupraClient();
      const { sequence_number } = await supra.account.getAccountInfo({
        accountAddress: sender,
      });

      // Type arguments may arrive as strings ("0x1::supra_coin::SupraCoin")
      // or as ready TypeTag objects.
      const typeArgs: TxnBuilderTypes.TypeTag[] = runTimeParams.map((t) =>
        typeof t === 'string' ? new TypeTagParser(t).parseTypeTag() : t
      );
      ...
      const rawTxn = supra.transaction.build.rawTxnObject({
        ...
        function:
          `${moduleAddress}::${moduleName}::${functionName}` as MoveFunctionId,
        functionTypeArgs: typeArgs,
        functionArgs: (params ?? []) as Uint8Array[],
        optionalTransactionPayloadArgs: {
          txExpiryTime: BigInt(txExpiryTime),
        },
      });

      // StarKey expects the serialized raw transaction as hex in `data`.
      // `to` and `value` are unused for Move entry-function calls.
      const txHash: string | null = await provider.sendTransaction({
  ...
  /**
   * Signs `message` with the connected account.
   *
   * `nonce` is not sent to the wallet: StarKey's signMessage takes only
   * `{ message }` and signs exactly those bytes. Anything that must be bound to
   * the signature - a login nonce, an expiry - has to be part of `message`
   * itself (see authMessageFor). The parameter stays for source compatibility
   * with earlier copies of this hook.
   */
  const signMessage = async (
    ...
    // The signer is the account the caller passed (a just-connected account
    // that has not committed to state yet) or the connected account. Never
    // mutate the `accounts` state array in place to fake one.
    ...
    let response: { publicKey: string; signature: string; address?: string };
    try {
      response = await provider.signMessage({ message: hexMessage });
    } catch (error) {
      // A declined or closed prompt must release the latch, or every later
      // sign attempt without forceSign resolves undefined and never prompts.
      localStorage.setItem('isSigningWallet', 'false');
      throw error;
    }
  ...
  const signIn = async () => {
    ...
    if (provider && accounts.length) {
      const nonce = await fetch('/api/auth/nonce').then((r) => r.text());
      // authMessageFor(nonce), not a "revalidate" variant. create-jwt verifies
      // exactly one composition, so any other wording here can never verify:
      // revalidation returned 401 every time and the only way out was a full
      // reconnect. forceSign, because the isSigningWallet latch would otherwise
      // make this resolve undefined without ever prompting.
      const signature = await signMessage(
        authMessageFor(nonce),
        nonce,
        accounts[0],
      ...
      // The server sets the httpOnly cookie, exactly as the connect path does.
      // Earlier versions wrote the token to localStorage and set a cookie from
      // JavaScript with an `HttpOnly` flag - browsers ignore that flag from
      // script, so the token was readable by any injected code, and the JS
      // cookie shadowed the server-set one.
      await fetch('/api/auth/wallet-login', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (provider && accounts.length && !justRequestedRelative) {
          ...
          // Same composition the server verifies - see signIn above.
          const signature = await signMessage(
            authMessageFor(nonce),
            nonce,
            accounts[0],
  ...
  //
  // https://docs.starkey.app/supra/events
  //
  ...
  const handleStarkeyAccountSwitch = async (nextAccounts: string[]) => {
    ...
    try {
      ...
      // authMessageFor(nonce) and forceSign, for the same two reasons as
      // signIn: the server verifies exactly one composition, and the
      // isSigningWallet latch would otherwise resolve this undefined without
      // ever prompting.
