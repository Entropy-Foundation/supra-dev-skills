...
export const ConnectWalletHandler: React.FC<ConnectWalletHandlerProps> = ({
  ...
  // Latest onConnect, readable from the event listeners below without
  ...
  // Connect wallet function. `onConnect` is deliberately NOT called here: this
  // closure holds the `accounts` array from before the await, which is still
  // empty on a first connect, so a check on it never passed. The callback fires
  // from the hook's wallet-connected event instead (see below), which carries
  // the account that was actually authenticated.
  ...
  useEffect(() => {
    ...
    // The hook dispatches this with the account it just authenticated: on a
    // first connect, after a re-auth, and after an account switch in StarKey.
    const handleWalletConnected = (event: Event) => {
      const account = (event as CustomEvent<{ account?: string }>).detail?.account;
      if (account) onConnectRef.current?.(account);
    };
  ...
  useEffect(() => {
    const handleProfileUpdated = (event: CustomEvent<UserProfile>) => {
      ...
      if (sameAddress(starKeyWalletHook.accounts[0], address)) {
            ...
            {connectionStage === 'idle' ? (
              <>
                ...
                <div className="flex justify-center mb-8">
                  <img className="h-12 w-auto mx-auto" src={logo.src} alt="MyDApp" />
                </div>
