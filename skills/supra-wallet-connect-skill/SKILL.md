...
`supra-ts-sdk` is used by the hook itself: `sendRawTransaction` builds the raw
transaction with `supra.transaction.build.rawTxnObject(...)` and hands the hex
bytes to StarKey's `sendTransaction`. StarKey's own `createRawTransactionData()`
is deprecated in its docs and is no longer called.
...
All files referenced below live in this skill's `assets/` directory. Read each one and write it into the target project verbatim (Read, then Write, in Claude Code). Do not try to reconstruct them from memory — they contain carefully-handled edge cases (wallet event listeners, retried account reads, late extension injection) that are easy to break.
...
- Every sign-in path signs `authMessageFor(nonce)` from the same file - the
  base `AUTH_MESSAGE` with the server-issued nonce appended - and `create-jwt`
  verifies the same composition. Never sign the bare `AUTH_MESSAGE`: StarKey
  signs only the bytes it is given, so a signature over a constant string is a
  permanent bearer credential that can be replayed with any fresh nonce.
  Earlier versions also signed `'Sign message to revalidate login to …'` and
  `'Token Expiry: …'`, which the server never verifies, so revalidation
...
Inside `<WalletProvider>`, prefer `useSupraWalletContext()` from
`components/WalletProvider.tsx`: it returns the provider's single hook instance
instead of creating another one with its own event subscriptions and polls.
...
6. **Reload the page.** The user must stay signed in. A single `accounts()` read
...
- **`references/starkey-runtime-quirks.md`** — what the Starkey extension actually does: which events report an account switch, why the first `accounts()` read comes back empty, which provider methods are deprecated, why `changeNetwork` lies in the mobile dApp browser, address shapes, and detection that has no natural end. Read **before** debugging any connect, switch, or network problem, and before adapting the event or network code.
...
- **Use StarKey's current API names.** `accounts()` (not the deprecated `account()`), and build transactions with `supra-ts-sdk` instead of the deprecated `createRawTransactionData()`. The provider reference is https://docs.starkey.app/supra/api-reference; the old `getting-started/*` URLs no longer exist.
...
- **The wallet lies about two things, so verify by reading back.** `accounts()` answers empty for a moment after every page load on a wallet that is connected — retry before believing it. `changeNetwork` rejects *after* switching in the mobile dApp browser — read the chain back and let that decide, never the call's own answer.
...
- **The sign-in message lives in one file and carries the nonce.** `lib/auth-constants.ts` exports `AUTH_MESSAGE` and `authMessageFor(nonce)`; every client call site signs `authMessageFor(nonce)` and the `create-jwt` route verifies the same composition. The nonce must be inside the signed bytes - StarKey signs only the message it is given, so a signature over a constant string can be replayed forever with any fresh nonce. Sign the same composition on revalidation too; a "revalidate" variant can never verify.
...
- **The sign-in message must match exactly on client and server.** This is the #1 source of "why doesn't login work" bugs. Customize only `APP_NAME` and `TOS_URL` in `lib/auth-constants.ts`; never re-declare the string or the nonce format anywhere else.
