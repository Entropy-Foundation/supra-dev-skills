...
|---|---|
| **supra-move-development** | Writing / reviewing Move contracts, Supra framework, dVRF 3.0, Oracles, Automation, Digital Asset NFTs, deploying from the browser IDE (ide.supra.com) or the Docker CLI |
...
- Sends everyone to Docker, when https://ide.supra.com compiles and deploys the same package from the browser, signed by StarKey, on Testnet or Mainnet
...
## From idea to a live dApp (the fast path)

1. **Contract.** Ask Claude: *"Build me a Supra dApp: an on-chain tip jar where anyone posts a message with a small SUPRA tip."* The Move skill writes the module, `Move.toml`, and tests, following the compiler rules that break first-time contracts.
2. **Deploy without Docker.** Open https://ide.supra.com, import the package, **Compile**, **Connect StarKey**, **Faucet** (Testnet), **Deploy**. Switch the network selector to **Mainnet** and deploy again when you are ready. Walkthrough: `skills/supra-move-development/references/supra_ide_deploy.md`.
3. **Frontend.** Ask Claude: *"Add a Next.js frontend with StarKey connect that calls the contract."* The wallet-connect skill drops in the hook, connect modal and sign-in flow; the TS SDK skill covers reads and transaction building.
4. **Ship.** Deploy the frontend anywhere Next.js runs. On a phone the connect modal offers **Open in Starkey**, which reopens the page inside the wallet's in-app browser.

The Docker CLI path (`scripts/setup_env.sh`, `scripts/deploy.sh`) remains for CI, scripting and Automation registration.
...
│   │   ├── SKILL.md
│   │   ├── references/                   # Move deep-dives, IDE deploy guide, SDK guide, patterns
...
| **Deploy** | Supra Move IDE in the browser (StarKey-signed, Testnet + Mainnet, nothing to install) or the Docker CLI |
...
| **Gas** | Fee model, simulation with `rawTxn.simulate(account)` before submitting |
...
- Optional sign-in-with-wallet → JWT → httpOnly cookie flow (server nonce inside the signed message, signature bound to the claimed address, edge-runtime API routes)
- Transactions built with `supra-ts-sdk` and signed by StarKey - none of the provider methods StarKey has deprecated
...
- [Supra Move IDE](https://ide.supra.com) · [IDE docs](https://docs.supra.com/network/move/dev/supra-move-ide)
- [StarKey Wallet](https://starkey.app) · [StarKey provider API](https://docs.starkey.app/supra/api-reference)
